...
{
    return add_endline(std::string(R"(\{"timestamp":")") + kTimestampRe + R"(","logger":")" + name +
                       R"(","level":")" + level + R"(","process":[0-9]+,"thread":[0-9]+)" + tail +
                       R"(\})");
}
...
TEST_CASE("json_logger: basic print methods", "[json_logger]")
{
    ...
    REQUIRE(matches(oss.str(), expected_line("BasicPrintMethods", "error",
...
TEST_CASE("json_logger: with_properties adds bound properties without affecting parent", "[json_logger]")
{
    ...
    REQUIRE(matches(oss.str(),
                    expected_line("AddingProperties", "trace", R"(,"property1":321,"message":"Test 1")")));
    oss.str({});

    {
        ...
        logger1.info("Test 2");
        REQUIRE(matches(oss.str(),
                        expected_line("AddingProperties", "info",
                                      R"(,"property1":321,"message":"Test 2")")));
        oss.str({});
    ...
    logger1.info("Test 4");
    REQUIRE(matches(oss.str(),
                    expected_line("AddingProperties", "info", R"(,"property1":321,"message":"Test 4")")));
}
...
TEST_CASE("json_logger: with_properties can override an existing key", "[json_logger]")
{
    ...
    logger1.trace("Test");
    REQUIRE(matches(oss.str(),
                    expected_line("ReplaceProperties", "trace", R"(,"property":321,"message":"Test")")));
    oss.str({});

    logger2.trace("Test");
    REQUIRE(matches(oss.str(),
                    expected_line("ReplaceProperties", "trace", R"(,"property":false,"message":"Test")")));
}
...
TEST_CASE("json_logger: a per-call property can override a bound property", "[json_logger]")
{
    ...
    REQUIRE(matches(oss.str(),
                    expected_line("DuplicateProperties", "critical",
                                  R"(,"property":321,"message":"Test 1")")));
    oss.str({});
    ...
    REQUIRE(matches(oss.str(),
                    expected_line("DuplicateProperties", "critical",
                                  R"(,"property":false,"message":"Test 2")")));
    oss.str({});

    logger2.critical({"property", nullptr}, "Test 3");
    REQUIRE(matches(oss.str(),
                    expected_line("DuplicateProperties", "critical",
                                  R"(,"property":null,"message":"Test 3")")));
}
          ...
          "[json_logger][error_handler]")
{
    ...
    REQUIRE(matches(out, expected_line("FailureLogger", "warning",
